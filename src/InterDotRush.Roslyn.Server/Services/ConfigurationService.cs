...
using InterDotRush.Roslyn.CodeAnalysis.Diagnostics;
...
{
    private const string ConfigurationFileName = "InterDotRush.config.json";
    ...
    {
        if (section?.InterDotRush?.Roslyn == null)
        {
            currentClassLogger.Error("Configuration section is not a valid document.");
            return;
        }

        configuration = section.InterDotRush.Roslyn;
...
{
    [JsonPropertyName("InterDotRush")]
    public InterDotRushSection? InterDotRush { get; set; }
    // Other sections that not related to lsp
}
internal sealed class InterDotRushSection
{
