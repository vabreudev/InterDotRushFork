...
export class LanguageServerController {
  ...
  public static async activate(
    ...
    LanguageServerController.serverOptions = {
      command: path.join(
        ...
        "bin",
        "LanguageServer",
        "InterDotRush" + (process.platform === "win32" ? ".exe" : "")
      ),
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand(res.commandIdReloadWorkspace, () =>
        LanguageServerController.reload()
      )
    );
    context.subscriptions.push(
      vscode.commands.registerCommand(res.commandIdPickTargets, () =>
    ...
    context.subscriptions.push(
      vscode.workspace.onDidSaveTextDocument(async (e) => {
        ...
        if (extName !== ".csproj" && extName !== ".props") return;

        const value = await vscode.window.showWarningMessage(
          res.messageProjectChanged,
          res.messageReload
        );
        if (value === res.messageReload) LanguageServerController.reload();
      })
    );
    context.subscriptions.push(
      vscode.tasks.onDidStartTask((e) => {
        if (
          !Extensions.getSetting<boolean>(
            "InterDotRush.msbuild.collectDiagnosticsOnBuild",
            true
          )
        )
          // TODO: publish or remove this
          return;
        if (
          e.execution.task.source === res.extensionId &&
          e.execution.task.name === "Build"
        )
          LanguageServerController.client.sendNotification(
            "InterDotRush/solutionDiagnostics",
            {}
          );
      })
    );
  ...
  public static start() {
    LanguageServerController.client = new LanguageClient(
      res.extensionId,
      res.extensionId,
      LanguageServerController.serverOptions,
      LanguageServerController.clientOptions
    );
    LanguageServerController.client.onNotification(
      "InterDotRush/projectLoaded",
      (project: string) => {
  ...
  public static reload() {
    if (!LanguageServerController.running) return;

    const workspaceFolders = vscode.workspace.workspaceFolders?.map(
      (folder) => ({ uri: folder.uri.toString(), name: folder.name })
    );
    LanguageServerController.client.sendNotification(
      "InterDotRush/reloadWorkspace",
      {
  ...
  private static async showQuickPickTargets(): Promise<void> {
    ...
    await Extensions.putSetting(
      res.configIdRoslynProjectOrSolutionFiles,
      result,
      vscode.ConfigurationTarget.Workspace
    );
    if (LanguageServerController.running) LanguageServerController.reload();
  }
  private static async shouldQuickPickTargets(): Promise<boolean> {
    const projectOrSolutionFiles = Extensions.getSetting<string[]>(
      res.configIdRoslynProjectOrSolutionFiles
    );
    if (
      projectOrSolutionFiles !== undefined &&
    ...
    if (solutions.length === 1 || projects.length === 1) return false;
