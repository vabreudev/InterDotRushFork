  ...
  implements vscode.DebugConfigurationProvider
{
  public async resolveDebugConfiguration(
    ...
    if (!config.type && !config.request && !config.name) {
      ...
      config.preLaunchTask =
        folder === undefined ? undefined : `${res.extensionId}: Build`;
    }
  ...
  public async resolveDebugConfigurationWithSubstitutedVariables(
    ...
    if (
      ...
      config.processId = await vscode.commands.executeCommand(
        res.commandIdPickProcess
      );
  ...
  private static provideVsdbgConfiguration(config: vscode.DebugConfiguration) {
    if (
      config.launchSettingsFilePath === undefined &&
      config.request === "launch"
    )
      config.launchSettingsFilePath =
        DebugAdapterController.getLaunchSettingsPath();

    // https://github.com/JaneySprings/InterDotRush/issues/39
    ...
    if (config.justMyCode === undefined)
      config.justMyCode = Extensions.getSetting(
        res.configIdDebuggerProjectAssembliesOnly,
        false
      );
    if (config.enableStepFiltering === undefined)
      config.enableStepFiltering = Extensions.getSetting(
        res.configIdDebuggerStepOverPropertiesAndOperators,
        false
      );
    ...
    if (config.symbolOptions === undefined)
      config.symbolOptions = {
        searchPaths: Extensions.getSetting(
          res.configIdDebuggerSymbolSearchPaths
        ),
        searchMicrosoftSymbolServer: Extensions.getSetting(
          res.configIdDebuggerSearchMicrosoftSymbolServer,
          false
        ),
      };
    if (config.sourceLinkOptions === undefined)
      config.sourceLinkOptions = {
        "*": {
          enabled: Extensions.getSetting(
            res.configIdDebuggerAutomaticSourcelinkDownload,
            true
          ),
        },
      };
  ...
  private static provideNcdbgConfiguration(config: vscode.DebugConfiguration) {
    ...
    if (config.launchSettingsFilePath !== undefined) {
      /* https://github.com/JaneySprings/InterDotRush/issues/22 */
      const profile = DebugAdapterController.getLaunchProfile(
        config.launchSettingsFilePath,
        config.launchSettingsProfile
      );
      DotNetDebugConfigurationProvider.provideNcdbgConfigurationFromProfile(
        config,
        profile
      );
    }
  ...
  private static provideNcdbgConfigurationFromProfile(
    config: vscode.DebugConfiguration,
